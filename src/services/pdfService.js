...
// Función helper para generar nombre de archivo con formato: Guest_name_fecha
function generateFileName(guestName) {
  // Limpiar el nombre del huésped (remover caracteres especiales y espacios)
  ...
  // Obtener fecha actual en formato legible (YYYY-MM-DD)
...
export async function createPDF(formData) {
  try {
    ...
    // Generar nombre de archivo con formato: Guest_name_fecha
...
export async function generatePDF(formElement) {
  try {
    ...
    const signatureDataUrl = originalSignature
      ? originalSignature.toDataURL()
      : null;
    ...
    if (clonedSignaturePad && signatureDataUrl) {
      ...
      clonedSignaturePad.parentNode.replaceChild(
        signatureImg,
        clonedSignaturePad
      );
    }
    ...
    // Extraer el nombre del huésped del formulario
    const guestNameInput =
      formElement.querySelector('input[name="Guest Name"]') ||
      formElement.querySelector("#guest-name") ||
      formElement.querySelector('[data-field="Guest Name"]');
